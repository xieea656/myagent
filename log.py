...
os.makedirs(".xlink/logs", exist_ok=True)
def log_tool_call(name , args, result ,status):
    """写工具日志，返回行号"""
    path =".xlink/logs/" + datetime.datetime.now().strftime("%Y-%m-%d") + "_tools.jsonl"
    entry = {
...
def read_tool_log(name):
    """读工具日志"""
    path = f".xlink/logs/{name}_tools.jsonl"
...
def read_line(date, line_num, start_line=None, end_line=None):
    ...
    path = f".xlink/logs/{date}_tools.jsonl"
    with open(path, "r", encoding="utf-8") as f:
